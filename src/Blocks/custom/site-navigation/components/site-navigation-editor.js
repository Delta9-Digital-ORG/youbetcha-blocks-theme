					...
					{siteNavigationLinks.map((link, i) => {
						return (
							<ReOrderableItem
								...
								title={link?.text?.length > 0 ? link.text : <i>{__('New link', 'you-betcha-cannabis-theme')}</i>}
								preIcon={
									<div className='es-display-flex'>
										...
										>
											<TextControl
												label={__('Link text', 'you-betcha-cannabis-theme')}
												value={link?.text}
												onChange={(value) => {
											...
											<LinkInput
												label={__('URL', 'you-betcha-cannabis-theme')}
												url={link?.url}
