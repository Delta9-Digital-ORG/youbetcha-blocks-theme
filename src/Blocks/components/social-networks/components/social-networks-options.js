...
export const SocialNetworksOptions = (attributes) => {
	...
	const modeOptions = [
		{
			label: __('Link to social networks', 'you-betcha-cannabis-theme'),
			icon: icons.link,
			value: false,
		},
		{
			label: __('Share to social networks', 'you-betcha-cannabis-theme'),
			icon: icons.share,
			value: true,
		}
	];

	const pickerSectionTitle = socialNetworksShareMode ? __('Share targets', 'you-betcha-cannabis-theme') : __('Social networks', 'you-betcha-cannabis-theme');
	...
	return (
		<UseToggle {...generateUseToggleConfig(attributes, manifest, 'socialNetworksUse')}>
			{!hideModeSelector &&
				<OptionSelector
					icon={icons.gears}
					label={__('Mode', 'you-betcha-cannabis-theme')}
