...
export const ImageOptions = (attributes) => {
	...
	if (!hideImagePicker && !imageUrl[breakpoints[0]]) {
		return (
			<UseToggle {...generateUseToggleConfig(attributes, manifest, 'imageUse')}>
				<Control icon={icons.image} label={__('Image', 'you-betcha-cannabis-theme')} additionalLabelClasses='es-h-spaced' noBottomSpacing>
					<MediaPlaceholder
						labels={{
							title: __('Add an image', 'you-betcha-cannabis-theme'),
	...
	return (
		<UseToggle {...generateUseToggleConfig(attributes, manifest, 'imageUse')}>
			{!hideImagePicker &&
				<Responsive label={__('Image', 'you-betcha-cannabis-theme')} icon={icons.image}>
						...
						return (
							<div className='es-h-center es-items-end! es-gap-0!' key={breakpointName}>
								...
								<Button
									key={index}
									icon={icons.trashAlt}
									label={__('Remove image', 'you-betcha-cannabis-theme')}
			...
			<Section
				...
				icon={icons.design}
				label={__('Design & layout', 'you-betcha-cannabis-theme')}
			...
			>
				...
				{!hideFullSizeToggle &&
					<IconToggle
						icon={icons.expandXl}
						label={__('Fill container', 'you-betcha-cannabis-theme')}
						checked={imageFull}
			...
			<Section
				showIf={!hideAltText}
				icon={icons.a11y}
				label={__('Accessibility', 'you-betcha-cannabis-theme')}
				noBottomSpacing={!additionalControlsAfterA11y}
			>
				<TextControl
					label={<IconLabel icon={icons.altText} label={__('Alt text', 'you-betcha-cannabis-theme')} />}
					value={imageAlt}
					onChange={(value) => setAttributes({ [getAttrKey('imageAlt', attributes, manifest)]: value })}
					help={__('Describes the content of the image', 'you-betcha-cannabis-theme')}
