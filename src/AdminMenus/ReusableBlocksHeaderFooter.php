 ...
 *
 * @package YouBetchaCannabisTheme\AdminMenus
 */
...
class ReusableBlocksHeaderFooter extends AbstractAdminMenu
{
	...
	{
		return \esc_html__('Header & footer', 'you-betcha-cannabis-theme');
	}
	...
	public function registerWpSettings(): void
	{
		...
		\add_settings_field(
			self::HEADER_PARTIAL,
			\__('Header partial', 'you-betcha-cannabis-theme'),
		...
		\add_settings_field(
			self::FOOTER_PARTIAL,
			\__('Footer partial', 'you-betcha-cannabis-theme'),
			[$this, 'renderPartialSelector'],
			self::ADMIN_MENU_SLUG,
			self::SETTINGS_SECTION_NAME,
			[
	...
	public function renderPartialSelector($args): void
	{
		...
		if ($reusableBlocksQuery->have_posts()) {
			...
			<select id="<?php echo \esc_attr($args['label_for']); ?>" name="<?php echo \esc_attr($type); ?>">
				<option value="">
					&mdash; <?php \esc_html_e('None', 'you-betcha-cannabis-theme'); ?> &mdash;
				</option>
		...
		<?php } else { ?>
			<i><?php echo \esc_html__('No reusable blocks found.', 'you-betcha-cannabis-theme'); ?></i>
			<?php
		}
	}
